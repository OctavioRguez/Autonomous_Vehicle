...
class Camera:
    # Variables
    def __init__(self):
        self.__cam_port =  'nvarguscamerasrc !  video/x-raw(memory:NVMM), width=1280, height=720, format=NV12, framerate=60/1 ! nvvidconv flip-method=2 ! video/x-raw, format=BGRx ! videoconvert ! video/x-raw, format=BGR ! appsink drop=true'
...
# Stop Condition
def stop():
    camera.releaseCamera() # Release camera
...
if __name__=='__main__':
    ...
    camera_pub = rospy.Publisher("/video_source/raw", Image, queue_size = 60) # Publish the camera image
